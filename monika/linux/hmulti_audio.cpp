...
struct AudioDevice
{
    ...
    int64 framesPlayed;
    int32 cycle;
    bool audioOpen;         // host PulseAudio/PipeWire sink active (via servercall)
};
    ...
    switch (op)
    {
        ...
        case B_MULTI_BUFFER_EXCHANGE:
        {
            ...
            if (!dev->running)
            {
                ...
                dev->framesPlayed = 0;
                dev->cycle = 0;
                // Phase 3: bring up the host audio sink in hyclone_server.
                // Falls back to the clock-paced null sink if unavailable.
                dev->audioOpen = (GET_SERVERCALLS()->audio_open(
                    kRateHz, kChannels, kSampleSize * 8) == B_OK);
            }
            ...
            // ready buffer is buffer[dev->cycle] here.
            ...
            if (dev->audioOpen && dev->buffers != NULL)
            {
                ...
                // which paces playback -- so do NOT also SleepUntil (that would
                // double-pace and halve the effective rate).
                const void* ready = (const char*)dev->buffers
                    + (size_t)dev->cycle * bufferBytes;
                if (GET_SERVERCALLS()->audio_write(ready, bufferBytes) != B_OK)
                    dev->audioOpen = false;   // sink died: drop to clock pacing
            }
